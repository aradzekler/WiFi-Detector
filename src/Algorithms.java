...
public class Algorithms extends csvWriter{
	...
	public static double[] weightedCenterPointGrade() {
		...
		Vector<Vector<String>> Vector = recordsToVector(mac);
	...
	private static Vector<Vector<String>> recordsToVector(String mac)  {
	...
	private static void filterStrongest(Vector<Vector<String>> vector) {
		...
		for (int i = 0; i < vector.size(); i++) {
			for (int j = 3; j < vector.get(i).size() - 3; j+=3) {
		...
		while (vector.size() > 2) {
			for (int i = 0; i < vector.size(); i++) {
				...
				if (Integer.parseInt(vector.get(i).lastElement()) == max2) {
					vector.removeElementAt(i);
				}
				if (Integer.parseInt(vector.get(i).lastElement()) == max1) {
					vector.removeElementAt(i);
				}
			}
		}
